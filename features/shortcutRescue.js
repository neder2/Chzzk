(() => {
    ...
    // 네이티브 처리 여부를 확인하는 대기 시간. 키 입력 후 이 시간 안에 상태 변화가 없으면 미반응으로 본다.
    const PROBE_TIMEOUT_MS = 350;
    // dead 오판은 이중 토글로 이어지므로 미반응을 2회 관찰한 뒤에만 즉시 실행 모드로 전환한다.
    const MISSES_TO_CONFIRM_DEAD = 2;
    ...
    function resolveProbe(action, video, snapshot, startedAt) {
        ...
        if (lastHref !== location.href) return;
        // 프로브 도중 우리가 폴백을 실행했다면 그 변화가 섞이므로 이번 판정은 버린다.
        if (lastFallbackAt >= startedAt) return;
        ...
        missCount += 1;
        if (missCount >= MISSES_TO_CONFIRM_DEAD) pipelineState = "dead";
    ...
    function onKeyDown(event) {
        ...
        if (typeof action.observe !== "function") return;
