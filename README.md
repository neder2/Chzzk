...
전체 방송(`/lives`)과 카테고리 방송 목록에 **검색창**과 **필터 메뉴**를 추가합니다.

- **방송 목록 도구:** 제목/스트리머 검색창과 시청자·팔로워 기준 필터를 제공합니다.
- **전체 방송 태그 검색 숨김:** 전체 방송 목록의 기본 태그 검색창을 숨기고, 확장 검색/필터만 보이게 합니다.
- **팔로워 수 스티커:** 각 방송의 프로필 아래에 팔로워 수를 작게 표시합니다.
- **진행 시간 호버:** 썸네일에 마우스를 올리면 현재 방송이 얼마나 진행됐는지 보여 줍니다.
- **필터 버튼 기준값:** 팔로워 필터와 시청자/조회수 필터에는 각각 6개의 버튼 기준값을 직접 지정할 수 있습니다. 1번 값은 "이하" 버튼 기준이고, 이후 값들은 구간 버튼의 경계값으로 쓰입니다.
