...
- **팔로잉 호버 미리보기:** 사이드바 팔로잉 라이브에 마우스를 올리면 실제 라이브 미리보기 카드가 즉시 표시됩니다. 진행 시간은 `00:00:00` 형식으로 표시하고, 시청자 수나 별도 LIVE 배지는 표시하지 않습니다. 기본적으로 메인 플레이어의 볼륨과 음소거 상태를 따르며, `미리보기 소리` 옵션을 끄면 음소거로 재생합니다. 페이지에 플레이어 런타임이 없을 때는 라이브 페이지 프레임으로 대체합니다.
...
- 관련 설정: `팔로잉 목록 자동 새로고침`, `자동 새로고침 주기`, `팔로잉 호버 미리보기`, `미리보기 소리`, `긴 제목 툴팁`, `방송 목록 도구`, `전체 방송 태그 검색 숨김`, `팔로워 수 스티커`, `진행 시간 호버`, `팔로워 버튼 1~6`, `시청자/조회수 1~6`, `추가 조회 페이지`, `팔로워 묶음`, `동시 조회`, `조회 간격`.
...
| 미리보기 소리 (`followingPreviewAudioEnabled`)                | 메인 플레이어 볼륨·음소거 상태 따름      | 켜짐                                                 | —              |
