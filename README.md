...
### Whale에서 List-KR 필터 사용 시 플레이어 단축키가 동작하지 않을 때

Whale 브라우저에서 List-KR 필터를 사용하는 경우 스페이스(재생/일시정지), `m`(음소거), `t`(넓은 화면), `f`(전체 화면) 등 치지직 기본 단축키가 간헐적으로 동작하지 않을 수 있습니다.

이 증상이 있으면 Whale의 광고 차단/필터 설정에서 치지직을 예외 처리한 뒤, 치지직 탭을 새로고침해 주세요.
