...
| 기능                                                     | 하는 일                                                        | 기본값 |
| -------------------------------------------------------- | -------------------------------------------------------------- | ------ |
| [자동 화질 복구](#1-자동-화질-복구)                      | 입장 시 1080p로 맞추고, 화질이 떨어지면 복구                   | 켜짐   |
...
| [방송 목록 도구](#11-방송-목록-도구)                     | 방송 목록에 검색창·필터 메뉴·팔로워 수·진행 시간 배지 추가     | 켜짐   |
...
- **다시보기 댓글 탭:** 우측의 `라이브 채팅 다시보기` 디자인은 유지하면서 옆에 `댓글` 탭을 추가합니다. VOD 화면이 유휴 상태일 때 치지직 댓글 API에서 첫 10개를 미리 받아 두고, 댓글 탭을 열면 같은 결과를 바로 표시합니다. 패널을 내리면 모바일 웹 방식의 다음 offset을 사용해 이어 붙입니다. 등록순·최신순·인기순과 새로고침을 지원하고, 작성자·시각·본문·타임코드·첨부 이미지를 우측 폭에 맞춰 표시합니다. 타임코드를 누르면 패널과 영상은 그대로 둔 채 해당 시각으로 이동합니다. 댓글 작성·버프·신고 같은 상태 변경 조작은 하단 원본 댓글에서만 사용합니다.
- 관련 설정: `다시보기 댓글 탭`.
...
전체 방송(`/lives`)과 카테고리 방송 목록 위에 **검색창 · 결과 수 표시 · 필터 메뉴**를 추가하고, 각 방송 카드에 팔로워 수와 진행 시간 정보를 더합니다.
...
| `라이브 채팅 다시보기` / `댓글` 탭 | 다시보기 우측 패널       | 채팅 다시보기와 읽기 전용 댓글 목록 전환                    |
...
| 설정                                                         | 설명                                         | 기본값   | 범위     |
| ------------------------------------------------------------ | -------------------------------------------- | -------- | -------- |
| **다시보기 댓글 탭** (`vodCommentTabsEnabled`)               | 채팅 영역에서 치지직형 읽기 전용 댓글 표시   | 켜짐     | —        |
...
| 방송 목록 필터·미리보기                              | 전체 라이브(`/lives`), 카테고리 목록(`/category/.../lives`) |
...
저장되는 데이터(설정값, 시청 기록 등)는 **사용자의 브라우저에만 저장되며 개발자 서버로 전송되지 않습니다.** 시청 기록은 Chrome 동기화가 아닌 이 브라우저의 로컬 저장소에 보관되고, 설정 화면의 **전체 삭제** 또는 브라우저의 확장 프로그램/사이트 데이터 삭제 기능으로 지울 수 있습니다. 채팅 도구가 모은 채팅 메시지는 현재 페이지에서만 유지되며 저장소에 저장하지 않습니다. 통나무 보상 자동 받기는 치지직 로그인 세션으로 `log-power` API를 호출하며, 결과를 개발자 서버로 전송하거나 별도 저장하지 않습니다.
...
- 모든 JS 파일 최상단에는 역할·의존·구조를 설명하는 헤더 주석이 있으므로, 파일을 통독하기 전에 헤더부터 읽으면 됩니다.
...
오디오 컴프레서와 채팅 서버 시각 추출은 [cheese-knife](https://github.com/jebibot/cheese-knife)
(MIT License, Copyright (c) 2023- jebibot)의 구현을 참고하거나 파생한 코드를 포함합니다.
