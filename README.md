...
- **팔로잉 호버 미리보기:** 사이드바 팔로잉 라이브에 마우스를 올리면 `live-detail` 메타데이터와 탐색 hover와 같은 `auto-play-info` HLS 정보를 받아 카드 안에서 미리보기를 재생합니다. 미리보기 소리는 기본값으로 켜져 있으며 주 플레이어 볼륨을 따라가고, 브라우저가 소리 자동재생을 막으면 음소거로 먼저 재생한 뒤 카드 안 버튼으로 소리를 켤 수 있습니다. 진행 시간은 `00:00:00` 형식으로 표시하고, 새 창/iframe fallback은 사용하지 않습니다.
...
- 관련 설정: `팔로잉 목록 자동 새로고침`, `자동 새로고침 주기`, `팔로잉 호버 미리보기`, `호버 미리보기 소리`, `긴 제목 툴팁`, `방송 목록 도구`, `전체 방송 태그 검색 숨김`, `팔로워 수 스티커`, `진행 시간 호버`, `팔로워 버튼 1~6`, `시청자/조회수 1~6`, `추가 조회 페이지`, `팔로워 묶음`, `동시 조회`, `조회 간격`.
...
시청 중 치지직 페이지가 **통나무 보상 받기** 버튼을 실제로 표시하면, 설정한 지연 시간 뒤 버튼 상태를 다시 확인하고 한 번만 클릭합니다. 이 기능은 사용자 동작을 자동화하며, 기본값은 켜짐입니다.
...
| 스페이스 **길게 누르기**       | 다시보기 플레이어        | 누르고 있는 동안 설정한 배속으로 재생                       |
...
| **통나무 보상 자동 받기** (`rewardAutoCollectEnabled`) | 페이지가 표시한 통나무 보상 버튼을 한 번 클릭                | 켜짐     | —        |
...
| **팔로잉 호버 미리보기** (`followingPreviewTooltipEnabled`)   | 사이드바 팔로잉 라이브의 방송 미리보기   | 꺼짐                                                 | —              |
| 호버 미리보기 소리 (`followingPreviewSoundEnabled`)           | 주 플레이어 볼륨으로 미리보기 소리 재생  | 켜짐                                                 | —              |
...
- **선택 호스트 권한** — `https://*.pstatic.net/*`: 팔로잉 미리보기 영상을 불러오는 네이버 CDN입니다. 설치할 때가 아니라 설정에서 **팔로잉 미리보기**를 켜는 순간에 허용 여부를 묻고, 한 번 허용하면 다시 묻지 않습니다.
...
│  ├─ followingPreviewTooltip.js
│  └─ liveWatchHistory.js
├─ icons/                   확장 프로그램 아이콘
├─ vendor/                  로컬 번들용 외부 런타임
