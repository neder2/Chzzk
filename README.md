...
| [통나무 보상 자동 받기](#9-통나무-보상-자동-받기)        | 페이지에 표시된 통나무 보상 버튼을 확인해 자동 클릭            | 켜짐   |
| [팔로잉 자동 새로고침](#10-팔로잉-사이드바)              | 사이드바 팔로잉 라이브 상태를 주기적으로 갱신                  | 켜짐   |
| [팔로잉 호버 미리보기](#10-팔로잉-사이드바)              | 팔로잉 라이브에 마우스를 올리면 카드 안에서 미리보기 재생      | 꺼짐   |
...
- 관련 설정: `스킵 기능`, `방향키 스킵`, `플레이어 버튼`, `라이브 플레이어 버튼`, `라이브 위치 유지`, `라이브 캐시 용량`, 그리고 `기본 스킵`/`휠 기본`/`Shift+휠`/`Alt+휠` 수치.
...
시청 중 치지직 페이지가 **통나무 보상 받기** 버튼을 실제로 표시하면, 설정한 지연 시간 뒤 버튼 상태를 다시 확인하고 보상 상태별로 한 번만 클릭합니다. 같은 보상의 버튼이 재렌더되더라도 중복 클릭하지 않으며, 다음 보상 상태나 다른 방송의 보상은 새로 수집합니다. 이 기능은 사용자 동작을 자동화하며, 기본값은 켜짐입니다.
...
- **팔로잉 호버 미리보기:** 사이드바 팔로잉 라이브에 마우스를 올리면 `live-detail` 메타데이터와 탐색 hover와 같은 `auto-play-info` HLS 정보를 받아 카드 안에서 미리보기를 재생합니다. 미리보기 소리는 기본값으로 켜져 있으며 주 플레이어 볼륨을 따라가고, 브라우저가 소리 자동재생을 막으면 음소거로 먼저 재생한 뒤 카드 안 버튼으로 소리를 켤 수 있습니다. 진행 시간은 `00:00:00` 형식으로 표시하고, 새 창이나 외부 프레임으로의 대체 재생은 사용하지 않습니다.
- 관련 설정: `팔로잉 목록 자동 새로고침`, `자동 새로고침 주기`, `팔로잉 호버 미리보기`(기본값: 꺼짐), `호버 미리보기 소리`.

> 팔로잉 호버 미리보기를 켜면 미리보기 영상을 불러오는 `pstatic.net` 호스트 권한을 그 시점에 요청합니다. [권한 및 개인정보](#권한-및-개인정보)를 참고하세요.
...
- 관련 설정: `방송 목록 도구`, `전체 방송 태그 검색 숨김`, `팔로워 수 스티커`, `진행 시간 호버`, `팔로워 버튼 1~6`, `시청자/조회수 1~6`, `진행 시간 1~6`, `추가 조회 페이지`, `팔로워 묶음`, `동시 조회`, `조회 간격`.
...
| **통나무 보상 자동 받기** (`rewardAutoCollectEnabled`) | 페이지가 표시한 통나무 보상 버튼을 확인해 자동 클릭          | 켜짐     | —        |
...
저장되는 데이터(설정값, 시청 기록 등)는 **사용자의 브라우저에만 저장되며 개발자 서버로 전송되지 않습니다.** 시청 기록은 Chrome 동기화가 아닌 이 브라우저의 로컬 저장소에 보관되고, 설정 화면의 **전체 삭제** 또는 브라우저의 확장 프로그램/사이트 데이터 삭제 기능으로 지울 수 있습니다. 채팅 도구가 모은 채팅 메시지는 현재 페이지에서만 유지되며 저장소에 저장하지 않습니다. 통나무 보상 자동 받기는 별도 보상 API를 호출하지 않고, 치지직 페이지가 실제로 표시한 버튼만 클릭합니다.
