...
- 관련 설정: `스킵 기능`, `방향키 스킵`, `플레이어 버튼`, `라이브 플레이어 버튼`, `라이브 위치 유지`, `라이브 캐시 용량`, `단축키 복구`, 그리고 `기본 스킵`/`휠 기본`/`Shift+휠`/`Alt+휠` 수치.
...
- 관련 설정: `팔로잉 목록 자동 새로고침`, `자동 새로고침 주기`, `팔로잉 호버 미리보기`, `방송 목록 도구`, `전체 방송 태그 검색 숨김`, `팔로워 수 스티커`, `진행 시간 호버`, `팔로워 버튼 1~6`, `시청자/조회수 1~6`, `추가 조회 페이지`, `팔로워 묶음`, `동시 조회`, `조회 간격`.
