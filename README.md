...
**Better Chzzk**는 자동 화질 복구, 다시보기 스킵/검색, 방송 목록 필터, 채널 방송 시간 통계, 라이브 시청 기록, 안내 팝업 정리 등의 기능을 한곳에 모은 확장 프로그램입니다. 모든 기능은 설정 화면에서 개별로 켜고 끌 수 있으며, 세부 수치까지 직접 조절할 수 있습니다.
...
- [기능 상세](#기능-상세)
    ...
    - [2. 다시보기·라이브 조작 (스킵)](#2-다시보기라이브-조작-스킵)
...
다시보기와 라이브 타임머신에서 원하는 만큼 시간을 건너뛸 수 있는 도구를 제공합니다.

- **방향키 스킵:** `←` / `→` 방향키로 설정한 **기본 스킵** 시간만큼 뒤로/앞으로 이동합니다. (입력창에 포커스가 있을 때나 Ctrl·Alt·⌘ 조합일 때는 동작하지 않습니다.)
...
- **일시정지 후 재생 위치:** 켜면 라이브를 일시정지했다 다시 재생할 때 멈췄던 위치에서 이어 봅니다. 끄면 치지직 기본 동작대로 실시간 위치로 이동합니다.
- 관련 설정: `스킵 기능`, `방향키 스킵`, `플레이어 버튼`, `라이브 플레이어 버튼`, `일시정지 후 재생 위치`, 그리고 `기본 스킵`/`휠 기본`/`Shift+휠`/`Alt+휠` 수치.
...
- **팔로워 수 표시:** 각 방송 카드의 프로필 이미지에 팔로워 수 배지를 붙입니다(예: `1.2만`, `5천`). 배지에 마우스를 올리면 정확한 수(예: `팔로워 12,345명`)를 보여 줍니다. 팔로워 수는 채널별로 추가 조회하며, 아래 조회 수치 설정으로 조회량과 속도를 조절합니다.
...
| 팔로워 배지 **호버**           | 방송 목록 카드           | 정확한 팔로워 수(툴팁) 표시                                 |
...
| 일시정지 후 재생 위치 (`skipLivePauseResumeEnabled`) | 멈춘 위치에서 이어 보기              | 켜짐     | —       |
| 다시보기 방송시각 (`vodBroadcastClockEnabled`)       | 장면의 실제 방송 시각 표시           | **꺼짐** | —       |
| 타임머신 지연 표시 (`timeMachineLagLabelEnabled`)    | 실시간 대비 지연 시간 표시           | 켜짐     | —       |
| 다시보기 채팅 복구 (`vodReplayChatFixEnabled`)       | 채팅 빠진 다시보기 한 번 새로고침    | 켜짐     | —       |
