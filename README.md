...
**Better Chzzk**는 자동 화질 복구, 다시보기 스킵/검색, 방송 목록 필터, 채널 방송 시간 통계, 라이브 시청 기록, 안내 팝업 정리 등의 기능을 한곳에 모은 확장 프로그램입니다. 대부분의 기능은 설정 화면에서 개별로 켜고 끌 수 있으며, 세부 수치까지 직접 조절할 수 있습니다.
...
- [기능 상세](#기능-상세)
    ...
    - [6. 안내 팝업 정리](#6-안내-팝업-정리)
    - [7. 채널 방송 시간 통계](#7-채널-방송-시간-통계)
    - [8. 다시보기 검색](#8-다시보기-검색)
    - [9. 방송 목록 필터](#9-방송-목록-필터)
    - [10. 라이브 시청 기록](#10-라이브-시청-기록)
- [시청 기록 페이지](#시청-기록-페이지)
...
- **저장:** 값을 바꾼 뒤 아래쪽 **설정 저장** 버튼을 누릅니다.
- **초기화:** **기본값 복원** 버튼을 누르면 모든 설정이 기본값으로 되돌아갑니다.
- **적용 시점:** 저장한 설정은 열려 있는 치지직 탭에 새로고침 없이 곧바로 반영됩니다.

![설정 화면](store-assets/screenshot-options-1280x800.png)
...
- **라이브 플레이어 버튼:** 라이브와 타임머신에서도 스킵 버튼을 표시합니다. 라이브 컨트롤에는 현재 버퍼의 라이브 끝 지점으로 이동하는 **빨리 감기** 버튼도 함께 제공됩니다.
...
시청 중에 뜨는 광고 차단 관련 안내 팝업과 어두운 배경(dimmed)을 닫고 화면에서 숨깁니다. 팝업 때문에 스크롤이 잠긴 경우도 함께 풀어 줍니다.
...
- 관련 설정: `방송 목록 도구`, `전체 방송 태그 검색 숨김`, `팔로워 수 스티커`, `진행 시간 호버`, `팔로워 버튼 1~6`, `시청자/조회수 1~6`, `추가 조회 페이지`, `팔로워 묶음`, `동시 조회`, `조회 간격`.
...
| **빨리 감기** 버튼             | 라이브 플레이어 컨트롤   | 현재 버퍼의 라이브 끝 지점으로 이동                         |
...
| 설정                                  | 설명                            | 기본값 |
| ------------------------------------- | ------------------------------- | ------ |
| **팝업 숨김** (`adblockPopupEnabled`) | 광고 차단 안내 팝업을 닫고 숨김 | 켜짐   |
...
| 기능                                  | 페이지                                                      |
| ------------------------------------- | ----------------------------------------------------------- |
| 자동 화질 복구, 스킵 조작             | 라이브(`/live/...`), 다시보기(`/video/...`)                 |
...
저장되는 데이터(설정값, 시청 기록 등)는 **사용자의 브라우저에만 저장되며 개발자 서버로 전송되지 않습니다.** 시청 기록은 Chrome 동기화가 아닌 이 브라우저의 로컬 저장소에 보관되고, 설정 화면의 **전체 삭제** 또는 브라우저의 확장 프로그램/사이트 데이터 삭제 기능으로 지울 수 있습니다.
...
### 광고 차단 프로그램 사용 시 플레이어 단축키가 동작하지 않을 때

**증상:** 스페이스(재생/일시정지), `m`(음소거), `t`(넓은 화면), `f`(전체 화면) 등 치지직 **기본 단축키**가 동작하지 않습니다. 키를 눌렀을 때 컨트롤 바는 나타나지만 실제 동작이 일어나지 않는 형태로 나타날 수 있습니다.

**원인:** AdGuard 같은 광고 차단 프로그램의 **한국어 필터(List-KR)**가 켜져 있을 때 간헐적으로 발생하는 현상으로 확인되었습니다. List-KR이 치지직에 적용하는 광고 추적 제거 규칙(`json-prune-xhr-response` 스크립틀릿)이 전역 `XMLHttpRequest`를 감싸는 과정에서, 페이지 진입 시점에 따라 치지직의 키 입력 → 재생 명령 파이프라인이 멈출 수 있습니다. Better Chzzk와는 무관하게(확장을 꺼도) 발생합니다.

#### 기본 대응 — 단축키 복구 기능 (자동)

Better Chzzk에는 **단축키 복구** 기능이 기본으로 켜져 있습니다. 단축키를 눌렀는데 치지직이 반응하지 않는 것이 감지되면, 그 키부터 확장이 대신 처리합니다(스페이스/`m`/`f`/`t`). 치지직이 정상 동작할 때는 전혀 개입하지 않습니다. 설정의 **플레이어 → 단축키 복구**에서 끌 수 있습니다.

자동 복구로 충분하지 않거나 근본적으로 해결하고 싶다면 아래 방법을 사용합니다.

#### 방법 1 — List-KR(한국어 필터)만 끄기

AdGuard 프로그램의 **설정 → 필터 → 언어별 필터**에서 **List-KR**을 끄고, 치지직 탭을 새로고침합니다.

> 한국 사이트 전반의 광고 차단이 약해지는 트레이드오프가 있습니다. 치지직만 문제라면 방법 2를 권장합니다.

#### 방법 2 — 치지직을 허용 목록(화이트리스트)에 추가

AdGuard 프로그램을 열고 **설정 → 허용 목록(화이트리스트)**에 다음 도메인을 추가한 뒤, 치지직 탭을 새로고침합니다.

```
chzzk.naver.com
```

> 이 방법은 치지직 페이지 전체에서 광고 차단·추적 방지가 꺼집니다. 다른 사이트에는 영향이 없습니다.

증상이 의심될 때 빠르게 확인하는 방법: 차단 프로그램을 잠시 통째로 끄고 치지직을 새로고침해서 단축키가 살아나는지 보면 됩니다. 살아난다면 위 방법으로 해결할 수 있습니다.
