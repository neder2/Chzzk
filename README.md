...
**원인:** AdGuard 같은 시스템 레벨 광고 차단 **프로그램**(브라우저 확장이 아닌 별도 설치형)이 치지직의 품질 측정 요청(`apis.naver.com/mcollector`)을 차단·변조하면, 치지직 플레이어의 키 입력 처리가 그 실패와 함께 중단되는 현상이 확인되었습니다. Better Chzzk와는 무관하게 발생하며, 차단 프로그램의 필터링 대상 설정에 따라 특정 브라우저에서만 나타날 수 있습니다.

Better Chzzk **1.1.6부터는 이 차단의 여파를 자동으로 막아 주지만**, 그래도 증상이 보이거나 이전 버전을 쓰는 경우 아래 방법으로 해결할 수 있습니다.

#### 방법 1 — 치지직을 허용 목록(화이트리스트)에 추가 (간단)
...
> 이 방법은 치지직 페이지 전체에서 광고 차단·추적 방지가 꺼집니다.

#### 방법 2 — 사용자 규칙으로 예외만 추가 (광고 차단 유지)

광고 차단은 유지하면서 문제가 되는 요청만 통과시키려면, AdGuard의 **설정 → 사용자 규칙(User rules)**에 아래 규칙을 추가합니다.

```
@@||apis.naver.com/mcollector^$domain=chzzk.naver.com
@@||apis.naver.com^$stealth,domain=chzzk.naver.com
@@||chzzk.naver.com^$stealth
```

- 첫 줄: 치지직에서 보내는 품질 측정 요청의 차단을 해제합니다.
- 둘째·셋째 줄: 스텔스 모드(추적 방지)가 해당 요청을 변조하지 않도록 예외 처리합니다. 스텔스 모드를 쓰지 않는다면 생략해도 됩니다.

규칙 저장 후 치지직 탭을 새로고침하면 적용됩니다.
