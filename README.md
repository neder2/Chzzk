...
- **팔로잉 호버 미리보기:** 사이드바 팔로잉 라이브에 마우스를 올리면 `live-detail` 메타데이터와 탐색 hover와 같은 `auto-play-info` HLS 정보를 받아 카드 안에서 음소거 미리보기를 재생합니다. 진행 시간은 `00:00:00` 형식으로 표시하고, 새 창/iframe fallback은 사용하지 않습니다.
