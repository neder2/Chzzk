...
- **방송 중 모아보기 유지:** 옵션을 켜면 보고 있는 동안 모은 방송자/채팅 운영자 채팅만 같은 방송 재입장 때 다시 보여 줍니다. 나가 있는 동안 새로 올라온 채팅을 가져오지는 않습니다.
...
- 관련 설정: `채팅 도구`(기본값: 꺼짐), `블라인드 채팅 원문 표시`, `방송자/채팅 운영자 모아보기`, `방송 중 모아보기 유지`, `모아보기 보관 개수`.
...
저장되는 데이터(설정값, 시청 기록 등)는 **사용자의 브라우저에만 저장되며 개발자 서버로 전송되지 않습니다.** 시청 기록은 Chrome 동기화가 아닌 이 브라우저의 로컬 저장소에 보관되고, 설정 화면의 **전체 삭제** 또는 브라우저의 확장 프로그램/사이트 데이터 삭제 기능으로 지울 수 있습니다. 채팅 도구가 모은 채팅 메시지는 기본적으로 현재 페이지에서만 유지하며, **방송 중 모아보기 유지** 옵션을 켠 경우에만 방송자/채팅 운영자 모아보기 캐시를 이 브라우저의 로컬 저장소에 저장합니다. 통나무 보상 자동 받기는 별도 보상 API를 호출하지 않고, 치지직 페이지가 실제로 표시한 버튼만 클릭합니다.
