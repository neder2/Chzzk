...
> **상태: 역사적 계획 문서.** 이 문서는 초기 리팩토링 착수 당시의 진단과 줄 번호를 보존한다.
> 현재 구조·로드 순서·테스트·작업 규칙은 `AGENTS.md`, `manifest.json`, `package.json`과 실제 소스를 우선한다.
> 아래의 규모, "테스트 스위트 없음", 옵션 중복 및 진행 체크리스트는 현재 상태로 간주하지 않는다.
...
- **작성 당시 진단:** 구조는 건강함. 단, **(a) 개발 도구 전무, (b) 인프라 코드 파일 간 중복, (c) 일부 거대 단일 파일, (d) CSS 이중 관리, (e) 옵션 키 삼중 정의**가 누적되어 있음. 아래 Phase로 해소한다.
...
## 2. 작성 당시 검증 방법 (현재는 자동 테스트 있음)

작성 당시에는 자동 테스트가 없어 **각 Phase 완료 후 아래 수동 스모크 매트릭스를 통과**시키도록 했다. 현재는 `package.json`의 테스트·린트·포맷 검사와 `AGENTS.md`의 검증 절차를 먼저 따르고, 아래 표는 수동 회귀 범위 참고용으로만 사용한다.
...
function normalizeOptions(value = {}) {
    /* schema 순회 + kind별 normalizeInteger/Boolean/SkipSeconds */
}
```
