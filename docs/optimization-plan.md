...
> **상태: v1.2.0 기준 역사적 계획 문서.** 이후 여러 항목이 구현되거나 구조가 바뀌었으므로 현재 문제 목록이나 실행 대기열로 사용하지 않는다.
> 후속 최적화는 실제 소스와 테스트로 병목을 다시 측정한 뒤 별도 계획을 세우며, 현재 규칙은 `AGENTS.md`를 따른다.
...
| features/rewardAutoCollect.js       | :136-151, :64-83, :241-271                | (기본 on) 스캔당 후보 수백 개 `innerText`(강제 스타일 계산) — 저렴한 필터 선행, 옵저버 타깃을 `aside#aside-chatting`으로 축소                                 | 높음(활성 시)   |
