...
> **상태: 1.2.2 기준 역사적 로드맵.** 이 문서는 더 이상 현재 작업 상태의 단일 공급원이나 실행 지시서가 아니다.
> 현재 구조·작업 규칙·검증 명령은 `AGENTS.md`, `manifest.json`, `package.json`, 실제 소스와 테스트를 우선한다.
...
- 작성 당시에는 각 세션 종료 시 이 문서의 해당 항목에 완료 표시(~~취소선~~ + 날짜)를 남겼다.
  현재 진행 상황은 실제 소스·테스트와 작업 요청을 기준으로 확인한다.
