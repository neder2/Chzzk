...
test("category metadata retry cooldown resets when the route changes", async () => {
    let requestCount = 0;
    const { dom, hooks } = evaluateFeature("categoryTools.js", {
        url: "https://chzzk.naver.com/category/game/first/lives",
        utils: {
            fetchJson: async () => {
                requestCount += 1;
                throw new Error("persistent metadata failure");
            },
        },
        hooks: `{
            ensureMetadata,
            getMetadataState: () => ({ key: metadataKey, retryAt: metadataRetryAt })
        }`,
    });
    const firstRoute = { scope: "category", categoryType: "game", categoryId: "first", tab: "lives" };
    const secondRoute = { scope: "category", categoryType: "game", categoryId: "second", tab: "lives" };
    dom.window.Date.now = () => 20000;
    dom.window.setTimeout = () => 1;
    dom.window.clearTimeout = () => {};

    try {
        await hooks.ensureMetadata(firstRoute);
        await hooks.ensureMetadata(firstRoute);
        assert.equal(requestCount, 1);

        await hooks.ensureMetadata(secondRoute);
        assert.equal(requestCount, 2);
        assert.equal(hooks.getMetadataState().key, "game/second/lives");
        assert.equal(hooks.getMetadataState().retryAt, 21000);
    } finally {
        dom.window.close();
    }
});
...
test("adblock popup drops its scroll restore after the suppressed popup is removed", () => {
    let applyOptions = null;
    const { dom, hooks } = evaluateFeature("adblockPopup.js", {
        url: "https://chzzk.naver.com/live/channel-a",
        utils: {
        ...
        hooks: "{ removeAdsPopup, unlockBodyScrollIfOnlySuppressedPopups }",
    });
    const { document } = dom.window;
    const popup = document.createElement("div");
    popup.setAttribute("role", "alertdialog");
    popup.textContent = "광고 차단 확장 기능을 종료해 주세요";
    popup.setAttribute("data-betterchzzk-suppress-adblock-popup", "1");
    const backdrop = document.createElement("div");
    backdrop.className = "popup_dimmed__test";
    backdrop.setAttribute("data-betterchzzk-suppress-adblock-popup", "1");
    ...
    try {
        ...
        popup.remove();
        hooks.removeAdsPopup();
        applyOptions({ adblockPopupEnabled: false });

        assert.equal(document.body.style.overflow, "", "a removed modal must not restore its stale scroll lock");
    } finally {
        dom.window.close();
    }
});

test("monthly widget retries mounting when the host appears without another mutation", async () => {
    ...
    const { dom, hooks } = evaluateFeature("monthlyBroadcastTime.js", {
        ...
        hooks: `{
            mountWidget,
            setRuntimeInstalled: (value) => { runtimeInstalled = value; }
        }`,
    });
    const { document } = dom.window;
    const timers = [];
    ...
    try {
        hooks.setRuntimeInstalled(true);
        assert.equal(hooks.mountWidget(), null);
        const retryTimer = timers.find((timer) => timer.delay === 250 && !timer.cleared);
        assert.ok(retryTimer, "a host retry should be scheduled after the initial miss");

        const host = document.createElement("div");
        const followButton = document.createElement("button");
        followButton.textContent = "팔로우";
        host.appendChild(followButton);
        ...
        const widget = document.getElementById("betterchzzk-monthly-broadcast-time");
        assert.ok(widget);
