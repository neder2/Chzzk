...
test("manifest loads telemetry guard first in the main world", () => {
    const manifest = JSON.parse(readRepoFile("manifest.json"));
    const mainScript = manifest.content_scripts.find((entry) => entry.world === "MAIN");

    assert.ok(mainScript);
    assert.equal(mainScript.run_at, "document_start");
    assert.equal(mainScript.js[0], "features/telemetryGuardPage.js");
});

test("telemetry guard fakes success for blocked collector requests", async () => {
    const vm = require("node:vm");
    const sandbox = {
        location: { href: "https://chzzk.naver.com/live/abc" },
        URL,
        Response: class {
    ...
    sandbox.window.fetch = (url) => Promise.reject(new TypeError(`Failed to fetch ${url}`));
    ...
    vm.runInContext(readRepoFile("features", "telemetryGuardPage.js"), sandbox);

    const blocked = await sandbox.window.fetch("https://apis.naver.com/mcollector/qoe?x=1");
    assert.equal(blocked.status, 200);

    await assert.rejects(sandbox.window.fetch("https://apis.naver.com/other/api"));

    const xhr = new sandbox.window.XMLHttpRequest();
    xhr.open("POST", "https://apis.naver.com/mcollector/qoe");
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.send("{}");
    assert.ok(!xhr.openedNatively);
    assert.ok(!xhr.sentNatively);
    assert.equal(xhr.status, 200);
    assert.equal(xhr.readyState, 4);
    assert.deepEqual(xhr.events, ["readystatechange", "load", "loadend"]);

    const passthrough = new sandbox.window.XMLHttpRequest();
    passthrough.open("GET", "https://api.chzzk.naver.com/service");
    passthrough.send();
    assert.ok(passthrough.openedNatively);
    assert.ok(passthrough.sentNatively);
});
