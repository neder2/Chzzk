...
test("global lives tag-search hide targets the container and stays stable around event popups", async () => {
    const chrome = createFakeChrome();
    const dom = createPageDom(
        [
            "<!doctype html>",
            "<body>",
            '<div class="navigation_component_tab"><div class="tab_list" id="tabLine">',
            '<button id="LIVE" type="button">라이브</button>',
            '<button id="VIDEO" type="button">동영상</button>',
            "</div></div>",
            '<div class="navigation_component_filter" id="filterLine">',
            '<div id="chips"><button type="button">인기</button><button type="button">최신</button><button type="button">추천</button></div>',
            '<div class="navigation_lives_container" id="livesContainer">',
            '<div class="navigation_lives_form" id="livesForm">',
            '<input id="tagInput" placeholder="태그 검색">',
            "</div>",
            "</div>",
            "</div>",
            "</body>",
    ...
    const { document } = dom.window;
    const rect = (el, top, left, width, height) => {
        el.getBoundingClientRect = () => ({ top, left, width, height, right: left + width, bottom: top + height });
    };
    rect(document.getElementById("tabLine"), 170, 270, 1200, 43);
    rect(document.getElementById("LIVE"), 170, 270, 74, 43);
    rect(document.getElementById("VIDEO"), 170, 344, 74, 43);
    rect(document.getElementById("filterLine"), 213, 270, 1200, 76);
    rect(document.getElementById("chips"), 220, 270, 200, 36);
    for (const chip of document.getElementById("chips").children) rect(chip, 220, 270, 60, 36);
    const container = document.getElementById("livesContainer");
    const form = document.getElementById("livesForm");
    rect(container, 238, 1290, 200, 36);
    rect(form, 238, 1290, 200, 36);
    rect(document.getElementById("tagInput"), 238, 1300, 180, 36);
    ...
    try {
        ...
        assert.equal(container.getAttribute("data-bcgt-hidden-tag-search"), "1", "태그 검색 전용 컨테이너를 숨긴다");
        assert.equal(form.hasAttribute("data-bcgt-hidden-tag-search"), false, "안쪽 폼이 아니라 컨테이너를 숨긴다");

        // 기간제 이벤트 팝업이 컨테이너에 끼어 높이가 임계값을 넘겨도 숨김 대상이 컨테이너로 유지돼야 한다.
        // (이전에는 높이 휴리스틱이 깨져 폼으로 바뀌며 컨테이너가 노출돼 목록이 밀렸다.)
        const popup = document.createElement("div");
        popup.id = "event-popup";
        popup.textContent = "놓치면 후회할 무료 치즈";
        container.appendChild(popup);
        rect(popup, 274, 1290, 190, 48);
        rect(container, 238, 1290, 200, 84);
        await new Promise((resolve) => setTimeout(resolve, 600));

        assert.equal(container.getAttribute("data-bcgt-hidden-tag-search"), "1", "팝업이 끼어 높이가 변해도 컨테이너 숨김이 유지된다");
        assert.equal(form.hasAttribute("data-bcgt-hidden-tag-search"), false);
        assert.equal(popup.isConnected, true, "이벤트 팝업 요소 자체는 건드리지 않는다");
    } finally {
        for (const listener of chrome.testState.storageChangeListeners) {
